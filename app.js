...
bot.action('service_trial', async (ctx) => {
  try {
    ...
    // 1. Cek apakah sudah ambil trial hari ini
    const hasAccess = await checkTrialAccess(userId);
    if (hasAccess) {
      return ctx.answerCbQuery('❌ Kuota Harian Habis! Silakan coba lagi besok.', { show_alert: true });
    }

    await ctx.answerCbQuery();
    
    // 2. Buka Menu Pilih Server (Mode: Trial, Type: SSH/UDP)
    await startSelectServer(ctx, 'trial', 'ssh');
    
  } catch (error) {
    logger.error("service_trial error:", error);
